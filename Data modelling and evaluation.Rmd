...
ydata <- select(trips_stats_fl, VESSEL_TYPE)
```
...
# Apply boxcox. TODO: Decide which transformation is more suitable. 
predict_boxcox <- preProcess(as.matrix(xdata),method = "BoxCox")
xdata_boxcox <- predict(predict_boxcox, as.matrix(xdata))  %>% data.frame()
```
