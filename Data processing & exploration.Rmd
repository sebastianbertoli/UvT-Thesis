...
measurements. *Footnote: Note that sampling rates that occur in less than 0.5% of the
sampled data were excluded.*

We can seethat the most frequent time intervals are concentrated within a small
range of values. That is for most vessels in this subset we have a measurement
or datapoint between every 2 and every 4 minutes or between every 20 and every
24 minutes.

Importantly the time interval between subsequent measurements is for most 
datapoints over 20 minutes. This is in stark contrast to the nominal AIS 
reporting intervals which typically are either every 3 minutes or every 10 
minutes. (Source: http://arundale.com/docs/ais/ais_reporting_rates.html). A
possible reason for this could be that the data provided by Greenpeace had been
sampled down to include fewer updates. 

There are two takeaways from this. First, we set the time treshold used to determine separate trips to 30 minutes. Second, the rather long update intervals between position measurements may determine the method we will be able to use.

Of course there are some limitations... (TODO Add limitations.)

CONTINUE HERE!

```{r}
# Just checking what vessels have 30 minutes time differences
