...
trajectory is broken up into two 'trips': a departure-trip and a return trip. 
This is necessary because a vessel may leave the port of Las Palmas on a Monday 
and only return several days later. Not doing this transformation leads to 
problems such as sudden jumps in the vessel-position visible (visible from the straight diagonal line in the Tracks plot). The transformation from a 
vessel trajectory to trips is implemented in the `Trajectory2Trips()` function 
below.
...
Before we can convert the trajectory into trips we need to have a closer look at the data. 
```{r}
