...
aisdata <- aisdata %>% 
  ...
  group_by(MMSI) %>% 
  filter(n() > 10) %>%  # TODO Try turning this one off.
  ungroup() %>%
...
# Just keep the categfories we are actually interested in
trips_stats_fl <- filter(trips_stats, VESSEL_TYPE %in% categories)
...
There are 4 approaches on how to deal with such class imbalances. The first is
to 
```{r}
select(trips, VESSEL_TYPE, TRIP_ID) %>% 
  unique() %>% 
  count(VESSEL_TYPE) %>%
  filter(n > 50) %>%
  ggplot() +
  ...
  coord_flip() + thm + theme(aspect.ratio = .5)
```
...
plot_speeds_box <- filter(trips_fl, AVG_SPEED > 1) %>% 
...
plot_speeds_density <- trips_fl %>% 
  filter(AVG_SPEED > 1, AVG_SPEED < 10, IN_PORT=="In Port") %>% 
  ggplot() +
...
### How long is a trip? 
A trip starts from the moment a vessel enters the captured area and ends either
when there is no position updadte for more than 30 minutes indicating that a 
vessel has left the area. Typically a vessel will dock at the port, stay there
for some time and depart again. Below we plotted the distribution of trip duration per type of vessel. We can see that drip durations vary wildly for some type of
vessels whereas for others they are less. The most important finding here is
that High Speed Vessels have the most regular pattern when it comes to the trip
duration which is consistently under 2.5 hours. 

```{r, echo=FALSE, message=FALSE, warning=FALSE}
# tmp <- nrow(filter(trips_stats_fl, duration_total_minutes < 6000)
plot_tripduration <- trips_stats_fl %>%
  ggplot(aes(VESSEL_TYPE, duration_total_minutes/60/24, color=VESSEL_TYPE)) +
...
```{r, echo=FALSE, message=FALSE, warning=FALSE}
# tmp <- nrow(filter(trips_stats_fl, duration_total_minutes < 6000)
plot_time_in_port <- trips_stats_fl %>%
  ggplot(aes(VESSEL_TYPE, area_15/60/60, color=VESSEL_TYPE)) +
  geom_jitter(alpha=0.2, position=position_jitter(0.3)) +
  geom_boxplot(outlier.alpha = 0, alpha = 0.1, 
               fill='black', color = 'black', width=0.2) +
  labs(title = 'Time spent in the port-area',
...
Below we have plotted the number of unique trips carried out by the vessels of 
each vesseltype. What we are interested here is not the absolute number of trips
carried out by each vessel but on the contrary the what the traffic patterns for
each individual vessel type is. For instance we can see that pleasure crafts
(e.g. sailing vessels) tend to travel less mid-week as opposed to the weekend or
beginning of the week. Ohter vessel types also show different travel patterns
throughout the different weekdays, however they appear to be less pronounced.
```{r}
tmp <- trips_fl %>% 
...
plot_tripsperday <- ggplot(data = tmp) +
  geom_line(mapping = aes(x=week_day, y=n, colour=VESSEL_TYPE))  + 
...
write_tsv(trips_stats_fl,'./dataset/out/trips_stats_fl.tsv')
...
ggsave("figures/plot_tdiff.png", plot = plot_tdiff)
...
ggsave("figures/plot_tripduration.png", plot = plot_tripduration)
ggsave("figures/plot_time_in_port.png", plot = plot_time_in_port)
ggsave("figures/plot_tripsperday.png", plot = plot_tripsperday)
