...
#### Detect trips
We divide a vessel's trajectory into a collection of trips based on the time
difference between subsequent measurements. In other words if a vessel does not
broadcast any position-data within a given time threshold it is assumed that the
vessel has left the area surrounding the port of Las Palmas.

First, we determine the time threshold  based on a subselection of the data.
This subselection is necessary to make sure that the threshold is ubiased due to
poor or missing position-data.
```{r}
tmp_trajectories_port <- trajectories %>%
  filter(START_LON > -15.6165, START_LON < -15.3933)  %>%
  filter(START_LAT > 28.1032, START_LAT < 28.1507) %>%
  filter(END_LON > -15.6165, END_LON < -15.3933)  %>%
  filter(END_LAT > 28.1032, END_LAT < 28.1507) %>%
  filter( (day(TIME_START) - day(TIME_END)) < 2 ) %>%   # To avoid day jumps.
  mutate(TIME_DIFF_MIN = round((TIME_DIFF/60)))
...
ggplot(data = tmp_test) + 
  ...
  labs(title = "Time differentials", 
       x = "Time between subsequent measurements [ minutes ]", y = "Proportion of data [ % ]")

# CONTINUE HERE

ggplot(data = tmp_trajectories_port) + 
  geom_density2d(mapping = aes(x=START_LAT,y=START_LON))
m <- ggplot(tmp_trajectories_port,aes(x=START_LON,y=START_LAT)) +
