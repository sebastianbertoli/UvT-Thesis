...
ggplot(data = trajectories) + 
  geom_histogram(mapping = aes(x = TIME_DIFF), bins = 75) +
  scale_x_log10() + scale_y_log10()

trajectories %>% 
  filter(LAT > -15.4165, LAT < -15.3933  )
  group_by(MMSI) %>% 
  summarise_each (funs(mean,median,sd), DISTANCE) %>% 
  ggplot() + 
  geom_histogram(mapping = aes(x = mean), bins = 100)
  # geom_histogram(mapping = aes(x = mean), bins = 100)



trajectories %>% group_by(MMSI) %>% summarise(mean(TIME_DIFF)) %>% 
  filter(TIME_DIFF > 600) %>% nrow()
```
