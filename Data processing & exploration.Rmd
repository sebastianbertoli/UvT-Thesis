...
#### How long is a trip? 
A trip is the duration from when a vessel enteres the Port of Las Palmas to the
moment it leaves the are or turns off its AIS transmitter for more than 15 minutes. Below we plotted the distribution of trip duration per type of vessel. We can seee that 

```{r, echo=FALSE, message=FALSE, warning=FALSE}
plot_tripdduration <- filter(trips_stats, VESSEL_TYPE %in% categories) %>%
  ggplot(aes(VESSEL_TYPE, duration_total_minutes, color=VESSEL_TYPE)) +
  geom_jitter(alpha=0.2, position=position_jitter(0.3)) +
  geom_boxplot(outlier.alpha = 0, alpha = 0.5, 
               fill='white', color = 'black', width=0.1, weight=0.1) +
  labs(title = 'Trip duration',
       x = 'Vessel Type', y = 'Trip Duration in Minutes') +
